...
export default async function Page({searchParams}:{searchParams?:{ideaId?:string}}){
  const i=searchParams?.ideaId??""; if(!i) return <div className="p-6"><h1 className="text-2xl font-semibold">Analytics & metrics</h1><p className="text-gray-600">Seleziona un’idea.</p></div>;
  ...
  return (<div className="p-6 space-y-6"><h1 className="text-2xl font-semibold">Analytics & metrics</h1>{rows.length?rows.map(r=>(
