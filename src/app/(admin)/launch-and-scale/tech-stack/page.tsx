...
export default async function Page({searchParams}:{searchParams?:{ideaId?:string}}){
  const i=searchParams?.ideaId??""; if(!i) return <div className="p-6"><h1 className="text-2xl font-semibold">Tech stack</h1><p className="text-gray-600">Seleziona un’idea.</p></div>;
  const s=await getSections(i); const t=new Set(["Technical Stack & Scalability Considerations"]); const rows=s.filter(x=>t.has(x.section));
  return (<div className="p-6 space-y-6"><h1 className="text-2xl font-semibold">Tech stack</h1>{rows.length?rows.map(r=>(
