...
export default async function KeyConsiderationsPage({ searchParams }: { searchParams?: { ideaId?: string } }) {
  ...
  const target = new Set(["Risk Assessment & Mitigation Plans", "Next Recommended Actions"]);
  const toShow = sections.filter(s => target.has(s.section));

  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-semibold">Key Considerations</h1>
      {toShow.length === 0 ? (
